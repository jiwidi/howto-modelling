"""Vendored A2D (autoregressive-to-diffusion) Qwen3 modeling code.

Copied verbatim from dllm-hub/Qwen3-0.6B-diffusion-bd3lm-v0.1 (Apache-2.0),
minus its `if __name__ == "__main__"` demo block. That block imports `dllm`,
which is not on PyPI, and transformers' static import scan refuses to load the
remote file because of it even though the block never runs.

Vendoring also means we load the model without trust_remote_code: importing this
module registers the classes with AutoConfig/AutoModelForMaskedLM.
"""
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        for decoder_layer in self.layers[: self.config.num_hidden_layers]:
            hidden_states = decoder_layer(
                hidden_states,
                # transformers 5.x dropped DecoderLayer.attention_type; this
                # checkpoint's config.layer_types is all "full_attention".
